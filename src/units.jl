...
"""
    parse_concentration(s::AbstractString) -> Quantity

Parse a concentration string into a Unitful quantity.
Handles "uM" -> "μM" substitution and whitespace.
"""
function parse_concentration(s::AbstractString)
    s = strip(s)
    isempty(s) && throw(ArgumentError("Empty concentration string"))

    s = replace(s, r"([0-9])(\s*)u([Mm])" => s"\1\2μ\3")
    s = replace(s, r"([0-9])(\s+)([a-zA-Zμµ])" => s"\1\3")

    local result
    try
        result = Unitful.uparse(s)
    catch e
        if e isa ArgumentError || e isa Base.Meta.ParseError
            throw(ArgumentError("Cannot parse concentration: \"$s\". Expected format like \"1.0M\" or \"500mM\""))
        end
        rethrow(e)
    end

    if !(result isa Unitful.Quantity) || Unitful.dimension(result) != Unitful.dimension(u"mol/L")
        throw(ArgumentError("Invalid concentration: \"$s\". Must have concentration units (e.g., M, mM, μM, mol/L)"))
    end
...
"""
    parse_time(s::AbstractString) -> Quantity

Parse a time string into a Unitful quantity. Handles "us" -> "μs" substitution.
"""
function parse_time(s::AbstractString)
    s = strip(s)
    isempty(s) && throw(ArgumentError("Empty time string"))

    s = replace(s, r"([0-9])(\s*)us\b" => s"\1\2μs")
    s = replace(s, r"([0-9])(\s+)([a-zA-Zμµ])" => s"\1\3")
    ...
    try
        ...
        if e isa ArgumentError || e isa Base.Meta.ParseError
            throw(ArgumentError("Cannot parse time: \"$s\". Expected format like \"500fs\" or \"2.5ps\""))
        end
        rethrow(e)
    end

    if !(result isa Unitful.Quantity) || Unitful.dimension(result) != Unitful.dimension(u"s")
        throw(ArgumentError("Invalid time: \"$s\". Must have time units (e.g., fs, ps, ns, μs, ms, s)"))
    end
